...
png(paste(dirname(myArgs[4]),"/tmp/percentiles.png", sep = ""))
centiles(m1,
         xvar = data_perc$x,
         ylim = c(ylim1,ylim2),
         xlim = c(xlim1,xlim2),
...
#resid_m1 <- residuals(m1, what= "z-scores", type = "simple")
...
write.csv(chart, file = paste(dirname(myArgs[4]),"/tmp/percentiles_chart.csv", sep = ""))

res_chart <- data.frame(resid_m1)
write.csv(res_chart, file = paste(dirname(myArgs[4]),"/tmp/res_chart.csv", sep = ""))